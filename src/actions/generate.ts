...
export const generateEntropyValue = async () => {
  ...
  console.log("Suggested values for username and password when setting up a remote node:");
  ...
  if (shouldStore) {
    const ipQuestion: any = [
      {
        ...
        validate: (input: string) => input.trim() !== "" || "IP address cannot be empty.",
      },
    ];
    ...
    await encryptAndStoreCredentials(nconfManager, remoteIp, "username", username);
    await encryptAndStoreCredentials(nconfManager, remoteIp, "password", password);

    console.log(`Credentials have been securely stored for remote node: ${remoteIp}`);
  } else {
